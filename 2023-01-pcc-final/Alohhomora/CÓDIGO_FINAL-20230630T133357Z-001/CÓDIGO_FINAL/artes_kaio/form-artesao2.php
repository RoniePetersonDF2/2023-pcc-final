...
if(isset($_POST['submit']))
{
    ...
    if(!empty($nome_artesao) && !empty($cpf) /*&& !empty($tipo_arte)*/ && !empty($email_artesao) && !empty($senha)  && !empty($confsenha)
    && !empty($telefone_artesao) && !empty($endereco_artesao) && !empty($idcoop) && !empty($nome_coop)){
        //cadastrar
      if($senha == $confsenha)
      {
        if(!$a->cadastrarArtesao($nome_artesao,$cpf,/*$tipo_arte,*/$email_artesao,$senha,$telefone_artesao,$endereco_artesao,$idcoop,$nome_coop))
        {
            
            echo "Existe um cadastro com esse endereço de EMAIL!";
            ?> <a href="listacoop.php"  class="btnform"><br> CLIQUE AQUI para preencher novamente o formulario</a><br><br><?php
        ...
        {
            header("location: pag.php?id_pg=$cpf");
...
?>
            ...
            <form action="form-artesao2.php" method="post">
                <fieldset>
                    ...
                    <div class="inputBox">
                        <input type="password" name="confsenha" id="confsenha" class="inputUser" maxlength="10" required>
